...
#define FILL_NORMAL_MASK_PX(color)                              \
    ...
    else *dest_buf = gfx_blend_color_mix(color, *dest_buf, *mask);     \
    mask++;                                                     \
    dest_buf++;
...
gfx_color_t gfx_blend_color_hex(uint32_t c)
{
    gfx_color_t r;
                ...
                if (*mask) {
                    ...
                    if (*mask != last_mask || last_dest_color.full != dest_buf[x].full) {
                        if (opa_tmp == OPA_COVER) {
                            ...
                            last_res_color = gfx_blend_color_mix(color, dest_buf[x], opa_tmp);
                        }
                        last_mask = *mask;
            ...
            if (mask == NULL || *mask) {
                ...
                if (mask == NULL || *mask != last_mask || last_dest_color.full != dest_buf[x].full || last_src_color.full != src_buf[x].full) {
                    if (opa_tmp == OPA_COVER) {
                        ...
                        last_res_color = gfx_blend_color_mix(src_buf[x], dest_buf[x], opa_tmp);
                    }
                    if (mask) {
                        last_mask = *mask;
                    }
