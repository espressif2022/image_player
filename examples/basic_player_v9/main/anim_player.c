...
static void anim_flush_cb(anim_player_handle_t handle, int x1, int y1, int x2, int y2, const void *data)
{
    ...
    lv_draw_buf_t *canvas_buf = lv_canvas_get_draw_buf(flush_canvas);
    if (canvas_buf == NULL) {
        anim_player_flush_ready(handle);
        return;
    }

    uint8_t *dst_data = lv_draw_buf_goto_xy(canvas_buf, x1, y1);
    const uint8_t *src_data = (const uint8_t *)data;
    
    uint32_t bytes_per_line = (x2 - x1) * 2;
    uint32_t canvas_stride = canvas_buf->header.stride;
    
    for (int y = y1; y < y2; y++) {
        lv_memcpy(dst_data, src_data, bytes_per_line);
        dst_data += canvas_stride;
        src_data += bytes_per_line;
    }
...
static void test_anim_player_common(const char *partition_label, uint32_t max_files, uint32_t checksum, uint32_t delay_ms)
{
    ...
    lv_draw_buf_t *canvas_draw_buf = lv_draw_buf_create(240, 400, LV_COLOR_FORMAT_RGB565, LV_STRIDE_AUTO);
    if (canvas_draw_buf == NULL) {
